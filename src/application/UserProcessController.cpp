...
UserProcessController::UserProcessController() : orderService() {}  // Order 제거에 따른 수정

void UserProcessController::handleMenu() {
    try {
        auto list = inventoryService.CallInventorySer();  // getList → CallInventorySer
        ui.displayList(list);
    } catch (const std::exception& e) {
        std::string err = e.what();
        errorService.logError(err);
        // errorService.notify_error(err); // 함수 없음
...
void UserProcessController::handlePayment(const std::string& cardInfo) {
    try {
        std::cerr << "[임시 처리] 결제 기능 생략됨\n";
        orderService.approve("paymentID", true); // 임시 승인 처리
        ui.displayMessage("결제 성공 (모의)");
    } catch (const std::exception& e) {
        std::string err = e.what();
        errorService.logError(err);
...
void UserProcessController::handlePrepayCode() {
    try {
        ...
        // bool usable = prepayService.isValid(code);  // 미구현
        bool usable = true;

        if (usable) {
            std::cerr << "[임시 처리] UC14 인증코드 유효 확인 완료\n";
            std::cerr << "[임시 처리] UC7 음료 배출 완료\n";
        }
    } catch (const std::exception& e) {
        std::string err = e.what();
        errorService.logError(err);
