...
TEST(InventoryRepositoryTest, SetAndGetAllDrinks) {
    ...
    inventoryRepository::setAllDrinks(testDrinks);
    const auto& result = inventoryRepository::getAllDrinks();

    for (size_t i = 0; i < result.size(); ++i) {
        EXPECT_EQ(result[i].getName(), testDrinks[i].getName());
        EXPECT_EQ(result[i].getPrice(), testDrinks[i].getPrice());
        EXPECT_EQ(result[i].getCode(), testDrinks[i].getCode());
    }
